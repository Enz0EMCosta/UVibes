...
function makePass(gl: WebGL2RenderingContext, frag: string, names: string[], defines: string): Pass | null {
...
function makeTarget(gl: WebGL2RenderingContext, w: number, h: number, fmt: Fmt): Target | null {
...
function makeDouble(gl: WebGL2RenderingContext, w: number, h: number, fmt: Fmt): DoubleTarget | null {
...
export default function InkFlowField(props: InkFlowFieldProps) {
    ...
    useEffect(() => {
        ...
        const advect = makePass(gl, FRAG_ADVECT, ["uVel", "uSrc", "uTexel", "uTexelSrc", "uDt", "uDiss"], defines)
        const divergence = makePass(gl, FRAG_DIVERGENCE, ["uVel", "uTexel"], "")
        const curl = makePass(gl, FRAG_CURL, ["uVel", "uTexel"], "")
        const vorticity = makePass(gl, FRAG_VORTICITY, ["uVel", "uCurl", "uCurlAmt", "uDt", "uTexel"], "")
        const pressure = makePass(gl, FRAG_PRESSURE, ["uPressure", "uDivergence", "uTexel"], "")
        const gradient = makePass(gl, FRAG_GRADIENT, ["uPressure", "uVel", "uTexel"], "")
        const clearPass = makePass(gl, FRAG_CLEAR, ["uTex", "uValue", "uTexel"], "")
        const splat = makePass(gl, FRAG_SPLAT, ["uTarget", "uAspect", "uColor", "uPoint", "uRadius", "uTexel"], "")
        ...
        const dispose = () => {
            const all: (Target | null)[] = [
                vel?.read ?? null, vel?.write ?? null,
                dye?.read ?? null, dye?.write ?? null,
                prs?.read ?? null, prs?.write ?? null,
                div, crl,
            ]
            for (const t of all) {
                if (!t) continue
        ...
        const onMove = (e: PointerEvent) => {
            ...
            if (p.moved) {
                p.dx += (nx - p.x) * (host.offsetWidth || 1)
                p.dy += (ny - p.y) * (host.offsetHeight || 1)
            }
            p.x = nx
            p.y = ny
            p.moved = 1
        }
        const onLeave = () => { pointer.current.moved = 0; pointer.current.down = 0 }
        ...
        const frame = (now: number) => {
            ...
            if (p.moved && (Math.abs(p.dx) > 0.01 || Math.abs(p.dy) > 0.01)) {
                ...
                doSplat(p.x, p.y, p.dx * gain, p.dy * gain, pick(colorPhase + p.x), radius, dtReal * INJECT_RATE)
            }
            p.dx = 0
            p.dy = 0
            ...
            if (L.drift > 0) {
                ...
                if (Math.abs(fx) > 0.01 || Math.abs(fy) > 0.01) {
                    doSplat(nx, ny, fx, fy, pick(colorPhase), radius * 0.8, dtReal * INJECT_RATE * (L.drift / 100))
                }
            }
